...
var crmdisco = require('./crmdiscoapi');
...
app.get('/getAToken', function(req, res) {
  ...
  authenticationContext.acquireTokenWithAuthorizationCode(req.query.code, redirectUri, resource, sampleParameters.clientId, sampleParameters.clientSecret, function(err, response) {
    ...
    authenticationContext.acquireTokenWithRefreshToken(response.refreshToken, sampleParameters.clientId, sampleParameters.clientSecret, resource, function(refreshErr, refreshResponse) {
      ...
      res.send(message); 
      var config = new crmdisco.crmdiscoapiconfig();
       config.APIUrl = 'https://globaldisco.crm.dynamics.com/api/discovery/v1.0/';
        config.AccessToken = refreshResponse.accessToken; 
        var api = new crmdisco.crmdiscoapi(config);
        api.GetInstances().then(function (results) {
            results.List.forEach(function(item){
              
         
              console.log("ORG Url:" + item.ApiUrl);
                 authenticationContext.acquireTokenWithRefreshToken(response.refreshToken, sampleParameters.clientId, sampleParameters.clientSecret, item.ApiUrl, function(error, refreshResponse) {
    
                
                console.log('calling whoami for org ' + item.ApiUrl) 
                  var apiconfig = { APIUrl:  item.ApiUrl +'/api/data/v8.1/', AccessToken: refreshResponse.accessToken };
                  var crmAPI = new CRMWebAPI(apiconfig);
                 
                       
               
                  crmAPI.ExecuteFunction('WhoAmI').then(function(r){
